...
module.exports = async (req, res) => {
  ...
  try {
    if (req.method === 'GET') {
      const rooms = await L.sb('GET', 'majlis_rooms?select=id,at,topic,created_by,open&order=id.desc&limit=50');
    ...
    if (req.method === 'POST') {
      ...
      const refMatch = topic.match(/(\d{1,3}):(\d{1,3})(?:-(\d{1,3}))?/);
      const verse = refMatch ? await L.fetchVerse(refMatch[0]) : null;
      const [room] = await L.sb('POST', 'majlis_rooms', { topic, verse, created_by: user.name });
